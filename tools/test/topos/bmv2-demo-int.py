    ...
    def __init__(self, args, **opts):
        ...
        for i in range(1, args.size + 1):
            for hostId in range(1, args.size + 1):
                host = self.addHost("h%d%d" % (i, hostId),
                                    cls=DemoHost,
                                    ip="10.0.0.%d%d/24" % (i, hostId),
                                    mac='00:00:00:00:%02x:%02x' % (i, hostId))
                self.addLink(host, bmv2Switches["s1%d" % i],
...
class DemoHost(ONOSHost):
    ...
    def config(self, **kwargs):
        super(ONOSHost, self).config(**kwargs)
        if len(self.intfs) > 1:
            ip = self.defaultIntf().IP()
            gateway = "10.0.0.254"
            self.bondIntfs( self.intfs[0], self.intfs[1] )
            self.cmd( 'ip addr add %s/24 dev %s' % (ip, self.bond0) )
            self.cmd( 'ip route add default via %s' % gateway )
